...
exports.getCourses = async (req, res) => {
  ...
  try {
    const { rows } = await connection.query(
      `SELECT c.course_id, c.name, c.description, c.created_at, c.subject, c.class_level,
      CASE 
...
exports.getSingleCourse = async (req, res) => {
  ...
  try {
    const { rows } = await connection.query(
      `SELECT c.course_id, c.price, c.name, c.description, c.created_at, c.subject, c.class_level,
      CASE 
      WHEN c.cover_image IS NOT NULL THEN CONCAT('http://localhost:8000/courses/image/', c.course_id)
      ELSE NULL
      END AS cover_image_url,
